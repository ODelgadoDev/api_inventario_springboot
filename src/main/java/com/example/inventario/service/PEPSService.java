...
public class PEPSService {
    ...
    public List<Map<String, Object>> generarReportePEPS(Integer productoId, LocalDateTime desde, LocalDateTime hasta) {
        ...
        entradas.sort(Comparator.comparing(Entrada::getFechaEntrada));
        salidas.sort(Comparator.comparing(Salida::getFechaSalida));

        LinkedList<Entrada> fifoEntradas = new LinkedList<>(entradas);
        ...
        for (Salida salida : salidas) {
            ...
            while (cantidadRestante > 0 && !fifoEntradas.isEmpty()) {
                Entrada entrada = fifoEntradas.peek();
